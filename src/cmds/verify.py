...
async def perform_action(ctx: ApplicationContext, reply):
    ...
    try:
        ...
        await reply(ctx, 'Whoops! I cannot DM you after all due to your privacy settings. Please allow DMs from other server members and try again.')
    except HTTPException:
        await reply(ctx, 'An unexpected error happened (HTTP 400, bad request). Please contact an Administrator.')
        return
