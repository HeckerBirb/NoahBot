...
from src.conf import MYSQL_URI, MYSQL_DATABASE, MYSQL_USER, MYSQL_PASS
    ...
    with connect(host=MYSQL_URI, database=MYSQL_DATABASE, user=MYSQL_USER, password=MYSQL_PASS) as connection:
        with connection.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                STDOUT_LOG.debug(f'Scheduled unban task for user_id {row[0]} at {run_at.strftime("%Y-%m-%d %H:%m:%S")}.')
